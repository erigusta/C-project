...
void main(){
  ...
  uint8_t sn = 0;
  	...
  	while ( ev3_search_sensor( LEGO_EV3_US, &sn, sn )) {
  		usSensor |= port_to_socket( ev3_sensor[ sn++ ].port );
  }
...
int getDistanceFwd(){//flera?
  ...
  return sensor_get_value(0, usSensor, 0);//returnerade bara det senare
